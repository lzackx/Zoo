...
#import "ZooMultiCaseListViewController.h"
#import <AFNetworking/AFNetworking.h>
@interface ZooMCViewController ()<DoraemonQRScanDelegate>
...
@implementation ZooMCViewController
...
- (void)wsDidiConnectHandle:(NSNotification *)notify {
    dispatch_async(dispatch_get_main_queue(), ^{
        self.clientCountLabel.text = [NSString stringWithFormat:@"当前连接从机数：%zd",[ZooMCServer connectCount]];
    });
}
...
- (void)startServer {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSError *error = nil;
        BOOL startServerSuccess = [ZooMCServer startServerWithError:&error];
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if(startServerSuccess) {
                ...
                [ZooToastUtil showToastBlack:@"服务开启成功" inView:self.view];
                ...
                if (error) {
                    [ZooToastUtil showToastBlack:error.localizedDescription inView:self.view];
                }
...
#else      //真机
        ZooQRScanView *scaner = [[ZooQRScanView alloc] initWithFrame:CGRectMake(0, self.bigTitleView.doraemon_bottom, self.view.doraemon_width, self.view.doraemon_height-self.bigTitleView.doraemon_bottom)];
...
- (void)caseListBtnClick{
    ZooMultiCaseListViewController  *caseListViewController = [[ZooMultiCaseListViewController alloc]init];
...
- (void)refreshUI {
    ...
    switch ([self workMode]) {
        ...
        case DoraemonMCViewControllerWorkModeServer:
        {
            ...
            NSString *url = [NSString stringWithFormat:@"http://%@:%zd/MyWs",[ZooAppInfoUtil getIPAddress:YES] , kDoraemonMCServerPort];
...
- (void)dealUrl:(NSString *)URL{
    [ZooMCClient connectWithUrl:URL];
}
