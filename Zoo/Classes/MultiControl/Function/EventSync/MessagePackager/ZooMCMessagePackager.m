...
#import "ZooMCGustureSerializer.h"
 ...
 */
+ (ZooMCMessage *)packageMessageWithView:(UIView *)view
                             gusture:(UIGestureRecognizer *)gusture
                              action:(SEL)action
                           indexPath:(NSIndexPath *)indexPath
                         messageType:(DoraemonMCMessageType)type {
    ZooMCMessage *messageInstance = [[ZooMCMessage alloc] init];
    ...
    switch (type) {
        ...
        {
            messageInstance.eventInfo =  [ZooMCGustureSerializer dictFromGusture:gusture];
            break;
        }
 ...
 * 自定义事件
 */
+ (ZooMCMessage *)packageCustomMessageWithView:(UIView *)view
 ...
 */
+ (ZooMCMessage *)parseMessageString:(NSString *)messageString {
