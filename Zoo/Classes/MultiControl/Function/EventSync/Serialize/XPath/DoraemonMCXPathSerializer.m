...
@property (nonatomic , strong) NSArray<ZooMCXPathNode *> *listContainerPathNodeList;

// 从根window到当前控件的XPath 如果isOneCell为true , 就代表从listContainer到当前控件的XPath
@property (nonatomic , copy ) NSArray<ZooMCXPathNode *> *pathNodeList;
...
- (void)parseView:(UIView *)view {
    ...
    NSMutableArray<ZooMCXPathNode *> *currentPathNodeList = [NSMutableArray array];
...
#pragma clang diagnostic pop
        ...
        ZooMCXPathNode *node = [[ZooMCXPathNode alloc] init];
    ...
    switch (components.count) {
        case 2:
        {
            ...
            [nodes enumerateObjectsUsingBlock:^(NSString *_Nonnull nodeString, NSUInteger idx, BOOL * _Nonnull stop) {
                ZooMCXPathNode *node = [ZooMCXPathNode nodeWithString:nodeString];
                if (node) {
                    [arrM addObject:node];
                }
            }];
...
- (NSString *)pathStringWitgNode:(ZooMCXPathNode *)node isLastNode:(BOOL)isLastNode{
...
- (NSString *)generalPathToTransfer {
    ...
    [self.pathNodeList enumerateObjectsUsingBlock:^(ZooMCXPathNode * _Nonnull node, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    if (self.isOneCell) {
        ...
        [self.listContainerPathNodeList enumerateObjectsUsingBlock:^(ZooMCXPathNode * _Nonnull node, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (BOOL)isMatchWithView:(UIView *)view node:(ZooMCXPathNode *)node {
    return YES;
}
...
+ (instancetype)nodeWithString:(NSString *)string {
    ...
    if (nodeComponents.count == 2) {
        ZooMCXPathNode *node = [[self alloc] init];
