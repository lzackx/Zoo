...
@implementation ZooMCEventHandler
...
- (BOOL)handleEvent:(ZooMCMessage*)eventInfo {
    ...
    if (eventInfo.currentVCClassName.length > 0 &&
        ...
        [ZooMCClient showToast:@"一机多控：收到主机手势消息，但当前页面与主机不一致，因此未执行此手势"];
        return NO;
    }

    if (self.targetView == nil) {
        [ZooMCClient showToast:@"一机多控：收到主机手势消息，但并未成功在当前页面找到对应控件，因此未执行此手势"];
        return NO;
    }
...
- (BOOL)handleEvent:(ZooMCMessage*)eventInfo {
    ...
    [ZooMCGustureSerializer syncInfoToGusture:ges withDict:data];
    ...
    if (DoraemonMCMessageTypeDidSelectCell == eventInfo.type ) {
        if ([rootView isKindOfClass:[UITableView class]]) {
            ...
            if (rowCount > targetIndexPath.row) {
                ...
                [ZooMCClient showToast:@"一机多控：收到主机点选列表项手势消息，但并未在从机上找到对应列表项，因此未执行此手势"];
            }
