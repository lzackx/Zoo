...
@implementation ZooMCReuseViewDelegateProxy
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([ZooMCServer isOpen]) {
        [ZooMCCommandGenerator sendMessageWithView:collectionView
                                                gusture:nil
                                                 action:nil
                                              indexPath:indexPath
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([ZooMCServer isOpen]) {
        [ZooMCCommandGenerator sendMessageWithView:tableView
                                                gusture:nil
                                                 action:nil
                                              indexPath:indexPath
                                            messageType:DoraemonMCMessageTypeDidSelectCell];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if ([ZooMCServer isOpen]) {
        ...
        if (lastIndexPath != nil) {
            [ZooMCCommandGenerator sendMessageWithView:scrollView
                                                    gusture:nil
                                                     action:nil
