...
@implementation ZooMultiURLProtocol

+ (ZooMultiURLSession *)shareSession {
    static dispatch_once_t onceToken;
    static ZooMultiURLSession  *session;
    dispatch_once(&onceToken, ^{
        session = [[ZooMultiURLSession alloc]initWithConfiguration:[NSURLSessionConfiguration defaultSessionConfiguration]];
    });
...
+ (BOOL)canInitWithRequest:(NSURLRequest *)request {
    ...
    if (![ZooMultiNetworkInterceptor shareInstance].shouldIntercept) {
        return NO;
    }
...
- (void)startLoading {
    ...
    if ([DoraemMultiMockManger sharedInstance].isResponseModifiy) {
        ...
        [[ZooMultiNetworkInterceptor shareInstance]handleResultWithRequest:[self request]
...
- (void)stopLoading {
    ...
    if ([DoraemMultiMockManger sharedInstance].isResponseModifiy) {
        ...
        [[ZooMultiNetworkInterceptor shareInstance] handleResultWithData:self.data
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveData:(NSData *)data {
    assert([NSThread currentThread] ==  self.clientThread);
    //弱网环境
    if ([ZooMultiNetworkInterceptor shareInstance].weakDelegate) {
        if(DoraemonMultiWeakNetwork_WeakSpeed == [[ZooMultiNetworkInterceptor shareInstance].weakDelegate weakNetSelecte]){
            [[ZooMultiNetworkInterceptor shareInstance].weakDelegate handleWeak:data isDown:YES];
        }
    }
