...
#endif
        self.xAxis = [[ZooXAxis alloc] init];
        self.yAxis = [[ZooYAxis alloc] init];
...
- (void)display {
    ...
    for (ZooChartDataItem *entry in self.items) {
...
- (void)drawBarsWithContext:(CGContextRef)context {
    ...
    for (NSUInteger i = 0; i < self.items.count; i++) {
        ZooChartDataItem *item = self.items[i];
