...
@interface ZooNetworkUtil()<NSURLSessionDelegate>

@end

@implementation ZooNetworkUtil

+ (nonnull ZooNetworkUtil *)shareInstance{
    static dispatch_once_t once;
    static ZooNetworkUtil *instance;
    dispatch_once(&once, ^{
        instance = [[ZooNetworkUtil alloc] init];
    });
    return instance;
}

+ (void)requestURL:(NSString *)url method:(NSString *)method param:(NSDictionary *)param success:(ZooNetworkSucceedCallback)successAction
...
+ (void)postWithUrlString:(NSString *)url params:(NSDictionary *)params success:(ZooNetworkSucceedCallback)successAction
                    error:(ZooNetworkFailureCallback)errorAction{
    [ZooNetworkUtil requestURL:url method:@"POST" param:params success:successAction error:errorAction];
}
...
+ (void)patchWithUrlString:(NSString *)url params:(NSDictionary *)params success:(ZooNetworkSucceedCallback)successAction error:(ZooNetworkFailureCallback)errorAction{
    [ZooNetworkUtil requestURL:url method:@"PATCH" param:params success:successAction error:errorAction];
}
