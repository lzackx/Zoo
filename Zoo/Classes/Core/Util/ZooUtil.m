...
+ (void)savePerformanceDataInFile:(NSString *)fileName data:(NSString *)data{
    ...
    NSString *anrDir = [cachesDir stringByAppendingPathComponent:@"ZooPerformance"];
...
+ (void)clearLocalDatas {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        for (NSString *folder in folders) {
            [ZooUtil clearFileWithPath:[homePath stringByAppendingPathComponent:folder]];
        }
    });
}
