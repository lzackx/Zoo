...
@interface ZooURLViewController () <UITableViewDelegate, UITableViewDataSource>
...
@implementation ZooURLViewController
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    self.dataSource = [[ZooCacheManager sharedInstance] urlHistoricalRecord];
...
- (void)jump {
    ...
    [[ZooCacheManager sharedInstance] saveURLHistoricalRecordWithText:url];
    if ([ZooManager shareInstance].urlRouterBlock) {
        [self leftNavBackClick:nil];
        [ZooManager shareInstance].urlRouterBlock(url);
    }
}
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    [[ZooCacheManager sharedInstance] clearURLHistoricalRecordWithText:self.dataSource[indexPath.row]];
    self.dataSource = [[ZooCacheManager sharedInstance] urlHistoricalRecord];
    [self.tableView reloadData];
}
...
/// 清除记录
- (void)clearRecord {
    [[ZooCacheManager sharedInstance] clearAllURLHistoricalRecord];
    self.dataSource = [[ZooCacheManager sharedInstance] urlHistoricalRecord];
    [self.tableView reloadData];
}
