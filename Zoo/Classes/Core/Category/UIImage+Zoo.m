...
@class ZooManager;
@implementation UIImage (Zoo)

+ (nullable UIImage *)zoo_imageNamed:(NSString *)name{
    if(name &&
       ![name isEqualToString:@""]){
        NSBundle *bundle = [NSBundle bundleForClass:NSClassFromString(@"ZooManager")];
        NSURL *url = [bundle URLForResource:@"Zoo" withExtension:@"bundle"];
...
+ (UIImage *)zoo_imageWithColor:(UIColor *)color {
    return [self zoo_imageWithColor:color size:CGSizeMake(1, 1)];
}
