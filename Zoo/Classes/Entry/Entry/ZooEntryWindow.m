...
@implementation ZooEntryWindow

- (UIButton *)entryBtn {
    if (!_entryBtn) {
        _entryBtn = [[UIButton alloc] initWithFrame:self.bounds];
        _entryBtn.backgroundColor = [UIColor clearColor];
        UIImage *image = [UIImage zoo_xcassetImageNamed:@"zoo_logo"];
#if defined(__IPHONE_13_0) && (__IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_13_0)
        if (@available(iOS 13.0, *)) {
            if (UITraitCollection.currentTraitCollection.userInterfaceStyle == UIUserInterfaceStyleDark) {
                image = [UIImage zoo_xcassetImageNamed:@"zoo_logo_dark"];
            }
        }
#endif
...
- (void)traitCollectionDidChange:(UITraitCollection *)previousTraitCollection {
    ...
    if (@available(iOS 13.0, *)) {
        if ([self.traitCollection hasDifferentColorAppearanceComparedToTraitCollection:previousTraitCollection]) {
            if (UITraitCollection.currentTraitCollection.userInterfaceStyle == UIUserInterfaceStyleDark) {
                [self.entryBtn setImage:[UIImage zoo_xcassetImageNamed:@"zoo_logo_dark"] forState:UIControlStateNormal];
            } else {
                [self.entryBtn setImage:[UIImage zoo_xcassetImageNamed:@"zoo_logo"] forState:UIControlStateNormal];
            }
        }
    }
#endif
...
- (instancetype)initWithStartPoint:(CGPoint)startingPosition{
    ...
    if (x < 0 || x > (ZooScreenWidth - _kEntryViewSize)) {
        x = defaultPosition.x;
    }
    
    if (y < 0 || y > (ZooScreenHeight - _kEntryViewSize)) {
        y = defaultPosition.y;
    }
...
- (void)showClose:(NSNotification *)notification{
    [_entryBtn setImage:[UIImage zoo_xcassetImageNamed:@"zoo_close"] forState:UIControlStateNormal];
...
- (void)closePluginClick:(UIButton *)btn{
    [_entryBtn setImage:[UIImage zoo_xcassetImageNamed:@"zoo_logo"] forState:UIControlStateNormal];
...
- (void)entryClick:(UIButton *)btn{
    if ([ZooHomeWindow shareInstance].hidden) {
        [[ZooHomeWindow shareInstance] show];
    }else{
...
- (void)normalMode: (UIPanGestureRecognizer *)panGestureRecognizer{
    ...
    if (newX > ZooScreenWidth - _kEntryViewSize/2) {
        newX = ZooScreenWidth - _kEntryViewSize/2;
    }
    if (newY < _kEntryViewSize/2) {
        newY = _kEntryViewSize/2;
    }
    if (newY > ZooScreenHeight - _kEntryViewSize/2) {
        newY = ZooScreenHeight - _kEntryViewSize/2;
    }
