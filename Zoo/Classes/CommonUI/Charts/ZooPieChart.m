...
- (void)drawRect:(CGRect)rect {
    ...
    double sum = 0;
    for (ZooChartDataItem *item in self.items) {
        sum += item.value;
    }
    ...
    [self.items enumerateObjectsUsingBlock:^(ZooChartDataItem * _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
